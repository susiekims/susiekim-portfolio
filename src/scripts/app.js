...
const changeEmoji = () => {
  $(".favicon").attr(
    "href",
    data.favicons[Math.floor(Math.random() * data.favicons.length)]
  );
...
const displayIcons = () => {
  data.icons.forEach(icon => {
    const altText = icon.split(".")[0];
    const iconHTML = `<li><img class="icon" src="../public/assets/icons/${icon}" alt="${altText}" title="${altText}"/></li>`;
...
const displayDev = () => {
  data.code.forEach(piece => {
    const newPiece = `<div class="piece scroll">
